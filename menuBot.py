...
class Menu:
    ...
    cur_menu = {}  # тут будет находиться текущий экземпляр класса, текущее меню
    ...
    def __init__(self, name, buttons=None, parent=None, module=""):
        ...
        markup.add(*buttons)  # Обратите внимание - звездочка используется для распаковки списка
        self.markup = markup
...
m_main = Menu("Главное меню", buttons=["Развлечения", "Игры", "Домашнее задание", "Помощь", "Календарь"])
m_games = Menu("Игры", buttons=["Игра КНБ", "Игра КНБ-MP", "Игра в 21", "Игра лишнее слово", "Выход"], parent=m_main, module="BotGames")
...
m_game_ext_word = Menu("Игра лишнее слово", buttons=['Хочу играть!', "Выход"], parent=m_games, module="BotGames")


m_DZ = Menu("Домашнее задание",
            buttons=["1 пункт", "2 пункт", "3 пункт", "4 пункт", "4.2 пункт", "5 пункт", "6 пункт", "7 пункт", "Выход"],
            parent=m_main, module="function_dz")
m_fun = Menu("Развлечения",
             buttons=["Прислать собаку", "Прислать лисичку", "Прислать анекдот", "Прислать новость", "Прислать фильм","Прислать курсы",
                      "Прислать предпросмотр сайта", "Угадай кто?", "Выход"], parent=m_main, module="fun")

m_calendar_ = Menu("Календарь", buttons=["Добавить событие", "Посмотреть события", "Выход"], parent=m_main, module="GoogleCalendarBot")

try:
    Menu.loadCurMenu()
except:
    # бывает что бот перезаписывает файл в момент отключения и в итоге остается пустой файл и не запускается, потому если не запускается, удаляем файл
    os.remove('bot_curMenu.plk')
    Menu.loadCurMenu()